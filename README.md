...
### 🏘️ VillagesML 自然村機器學習分析系統
基於廣東省 285,860 條自然村地名的綜合分析平台：

**數據規模：**
- 285,860 條自然村地名
- 21 個市級、121 個區縣級、1,446 個鄉鎮級行政區
- 9 個語義類別、多維度特徵向量

**核心模組：**

1. **搜尋探索** - 關鍵詞搜尋、三級行政區過濾、深度分析
2. **字符分析** - 字符嵌入、顯著性分析、共現網絡、字符傾向性
3. **語義分析** - 語義類別、子類別、語義指數、語義網絡
4. **空間分析** - 地理聚類、熱點識別、空間可視化、空間整合
5. **模式分析** - N-gram 探索、結構模式、模式傾向性
6. **區域分析** - 區域聚合、特徵向量、類別傾向、區域相似度
7. **ML 計算** - 聚類分析（K-Means/DBSCAN/GMM）、特徵提取、子集分析

**技術特點：**
- **GPU 加速地圖渲染** - MapLibre GL Symbol Layer，支持 10,000+ 標記
- **多種聚類算法** - K-Means、DBSCAN、GMM、層次聚類、空間感知聚類
- **語義分析** - 9 個主類別（農業、宗族、方位、基建、山地、聚落、象徵、植被、水系）
- **跨層級比較** - 支持市級、縣級、鄉鎮級的特徵向量比較
- **實時計算** - 前端交互式參數調整，後端實時計算返回結果

**[→ VillagesML 完整功能文檔](./docs/VillagesML/FEATURE_OVERVIEW.md)**
...
│   │   ├── explore/            # 分析工具（音韻、統計、Praat、VillagesML）
│   │   ├── auth/               # 認證頁面
│   │   └── intro/              # 關於/介紹頁面
│   │
│   ├── components/             # 可復用組件
...
│   │   ├── sql/                # 數據庫操作
│   │   ├── user/               # 用戶數據管理
│   │   ├── villagesML/         # VillagesML API 模塊
│   │   │   ├── search.js       # 搜尋 API
│   │   │   ├── character.js    # 字符分析 API
│   │   │   ├── semantic.js     # 語義分析 API
│   │   │   ├── spatial.js      # 空間分析 API
│   │   │   ├── pattern.js      # 模式分析 API
│   │   │   ├── regional.js     # 區域分析 API
│   │   │   ├── clustering.js   # 聚類分析 API
│   │   │   ├── compute.js      # ML 計算 API
│   │   │   └── index.js        # VillagesML API 導出
│   │   └── index.js            # 中央導出中心
│   │
│   ├── utils/                  # 工具函數
│   │   ├── store.js            # 響應式狀態管理
│   │   ├── villagesMLStore.js  # VillagesML 專用狀態
│   │   ├── message.js          # Toast 通知和對話框
│   │   └── constants.js        # 全局配置
│   │
│   ├── config/                 # 配置文件
│   │   └── villagesML.js       # VillagesML 配置（語義類別映射等）
│   │
...
#### VillagesML 分析 (/villagesML)
自然村機器學習分析系統，提供 7 大模組：
- **搜尋探索** - 關鍵詞搜尋、三級行政區過濾、深度分析
- **字符分析** - 字符嵌入、顯著性分析、共現網絡
- **語義分析** - 語義類別、子類別、語義指數、語義網絡
- **空間分析** - 地理聚類、熱點識別、空間可視化
- **模式分析** - N-gram 探索、結構模式、模式傾向性
- **區域分析** - 區域聚合、特徵向量、區域相似度
- **ML 計算** - 聚類分析、特徵提取、子集分析
...
- **v3.x**: 功能擴展期，引入工具、詞句、音系等（2026-01-26 ~ 2026-02）
- **v4.x**: VillagesML 系統上線（2026-03 ~ 至今）
